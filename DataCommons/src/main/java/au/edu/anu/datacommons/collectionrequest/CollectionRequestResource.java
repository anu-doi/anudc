...
@Scope("request")
@Path("/collreq")
public class CollectionRequestResource {
	private static final Logger LOGGER = LoggerFactory.getLogger(CollectionRequestResource.class);

	private static final String COLL_REQ_JSP = "/collreq.jsp";
	private static final String COLL_REQ_LIST_JSP = "/request/collection_request.jsp";
	private static final String COLL_REQ_SINGLE_JSP = "/request/collection_request_single.jsp";
	...
	@Resource(name="collectionRequestServiceImpl")
	...
	public Response doGetAsHtml() {
		List<CollectionRequest> collReqs = requestService.getUserRequests();
		List<CollectionDropbox> dropboxes = requestService.getUserApprovedRequests();
		
		Map<String, Object> model = new HashMap<String, Object>();
		model.put("collReqs", collReqs);
		model.put("approvedRequests", dropboxes);
		return Response.ok(new Viewable(COLL_REQ_LIST_JSP, model)).build();
	}
	...
	public Response doGetReqItemAsHtml(@PathParam("collReqId") Long collReqId) {
		...
		try {
			...
			Collections.sort(collReq.getAnswers(), new Comparator<CollectionRequestAnswer>() {
				@Override
				public int compare(CollectionRequestAnswer ans1, CollectionRequestAnswer ans2) {
					int compareVal = 0;
					
					if (ans1.getAnswerOrder() != null) {
						ans1.getAnswerOrder().compareTo(ans2.getAnswerOrder());
					}
					if (compareVal == 0) {
						compareVal = ans1.getId().compareTo(ans2.getId());
					}
					return compareVal;
				}
			});
			...
			resp = Response.ok(new Viewable(COLL_REQ_SINGLE_JSP, model)).build();
		}
	...
	public Response doPostCollReqAsHtml(@Context HttpServletRequest request, @Context UriInfo uriInfo,
		...
		try {
			...
			CollectionRequest collectionRequest = requestService.saveAnswers(fedoraObject, allFormParams, request.getRemoteAddr());

			uriBuilder = UriBuilder.fromPath("/collreq/").path(collectionRequest.getId().toString());
			...
			varMap.put("collReqUrl",
					uriInfo.getBaseUriBuilder().path(this.getClass()).path(this.getClass(), "doGetReqItemAsHtml")
							.build(collectionRequest.getId()).toString());
			...
			messages.add(MessageType.SUCCESS, "Collection Request successfully saved. ID# " + collectionRequest.getId(), model);
			model.put("collReq", collectionRequest);
			uriBuilder = uriBuilder
					.queryParam("smsg", "Collection Request saved. ID# " + collectionRequest.getId().toString());
		} catch (Exception e) {
	...
	public Response doPostUpdateCollReqAsHtml(@PathParam("collReqId") Long collReqId, @Context UriInfo uriInfo,
		...
		try {
			...
			if (status == ReqStatus.ACCEPTED && collReq.getItems().size() > 0) {
				...
				dropboxUri = UriBuilder.fromUri(uriInfo.getBaseUri()).path(CollectionRequestResource.class)
						.path(CollectionRequestResource.class, "doGetDropboxAccessAsHtml")
	...
	public Response doGetDropboxAccessAsHtml(@Context HttpServletRequest request, @Context UriInfo uriInfo,
		...
		try {
			...
			UriBuilder uriBuilder = uriInfo.getBaseUriBuilder().path(CollectionRequestResource.class)
					.path(CollectionRequestResource.class, "doGetDropboxFileAsOctetStream")
...
//		else if (task.equals("listCollReq")) {
			...
			CollectionRequestResponse responseQuestions = requestService.getQuestions(task, pid, groupId, domainId);
			responseQuestions.sortQuestionsByOrder();
			resp = Response.ok().entity(responseQuestions).build();
		}
