	...
	<p>
		<ul class="nobullet">
			<c:forEach items="${it.domains}" var="domain">
				<li>${domain.domain_name} [${domain.id}] <button id="btn-edit-${domain.id}" data-id="${domain.id}" data-name="${domain.domain_name}" class="edit-domain" title="Edit" aria-label="Edit ${domain.domain_name}" data-bs-toggle="modal" data-bs-target="#popupEditDomain"><img src="//style.anu.edu.au/_anu/images/icons/web/draw.png" alt="Edit" /></button>
					<button id="btn-delete-${domain.id}" class="delete-domain" data-id="${domain.id}" alt="Delete" aria-label="Delete ${domain.domain_name}" title="delete">
